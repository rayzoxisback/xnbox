...
[Library( "ent_car_homer", Title = "Homer", Spawnable = true )]
public partial class HomerEntity : Prop, IUse
{
	public static float homer_speed_divide = 0.5f; // 0.0f = low -- 0.5f medium -- 1.0f fast	

  	[ConVar.Replicated( "debug_car" )]

	public static bool debug_car { get; set; } = false;

	private HomerWheel frontLeft;
	private HomerWheel frontRight;
	private HomerWheel backLeft;
	private HomerWheel backRight;
	...
	public HomerEntity()
	{
		frontLeft = new HomerWheel( this );
		frontRight = new HomerWheel( this );
		backLeft = new HomerWheel( this );
		backRight = new HomerWheel( this );
	}
	...
	{
		base.Spawn();
		var mdl = "models/cars/Homer/homer";
		SetModel( mdl );
	...
	public override void ClientSpawn()
	{
...

		{
			...
			chassis_axle_front.LocalPosition = new Vector3( 2.2f, 0, 0.45f ) * 33.0f;
			clientModels.Add( chassis_axle_front );

			{
				wheel0 = new ModelEntity();
				wheel0.SetModel( "models/cars/Homer/homer_wheel" );
...

			{
				...
				wheel1.SetModel( "models/cars/Homer/homer_wheel" );
...

		{
			...
			chassis_axle_rear.LocalPosition = new Vector3( -1.7f, 0, 0.45f ) * 33.0f;
...

			{
				...
				wheel2.SetModel( "models/cars/Homer/homer_wheel" );
...

			{
				...
				wheel3.SetModel( "models/cars/Homer/homer_wheel" );
	...
	public void OnPrePhysicsStep()
	{
		...
		{
			var forwardSpeed = MathF.Abs( localVelocity.x );
			var speedFactor = homer_speed_divide - (forwardSpeed / 5000.0f).Clamp( 0.0f, 1.0f );
	...
	public bool OnUse( Entity user )
	{
		...
		{
			player.Vehicle = this;
			player.VehicleController = new HomerController();
			player.VehicleCamera = new HomerCamera();
