...
[Library( "ent_car_oldcar", Title = "Oldcar", Spawnable = true )]
public partial class OldcarEntity : Prop, IUse
{
	public static float oldcar_speed_divide = 1.0f; // 0.0f = low -- 0.5f medium -- 1.0f fast	

  	[ConVar.Replicated( "debug_car" )]

	public static bool debug_car { get; set; } = false;

	private OldcarWheel frontLeft;
	private OldcarWheel frontRight;
	private OldcarWheel backLeft;
	private OldcarWheel backRight;
	...
	public OldcarEntity()
	{
		frontLeft = new OldcarWheel( this );
		frontRight = new OldcarWheel( this );
		backLeft = new OldcarWheel( this );
		backRight = new OldcarWheel( this );
	}
	...
	{
		base.Spawn();
		var mdl = "models/cars/Oldcar/oldcar";
		SetModel( mdl );
	...
	public override void ClientSpawn()
	{
...

		{
			...
			chassis_axle_front.LocalPosition = new Vector3( 1.6f, 0, 0.53f ) * 34.0f;
			clientModels.Add( chassis_axle_front );

			{
				wheel0 = new ModelEntity();
				wheel0.SetModel( "models/cars/Oldcar/oldcar_wheel" );
...

			{
				...
				wheel1.SetModel( "models/cars/Oldcar/oldcar_wheel" );
...

		{
			...
			chassis_axle_rear.LocalPosition = new Vector3( -1.3f, 0, 0.53f ) * 34.0f;
...

			{
				...
				wheel2.SetModel( "models/cars/Oldcar/oldcar_wheel" );
...

			{
				...
				wheel3.SetModel( "models/cars/Oldcar/oldcar_wheel" );
	...
	public void OnPrePhysicsStep()
	{
		...
		{
			var forwardSpeed = MathF.Abs( localVelocity.x );
			var speedFactor = oldcar_speed_divide - (forwardSpeed / 5000.0f).Clamp( 0.0f, 1.0f );
	...
	public bool OnUse( Entity user )
	{
		...
		{
			player.Vehicle = this;
			player.VehicleController = new OldcarController();
			player.VehicleCamera = new OldcarCamera();
