...
[Library( "ent_car_quadbike", Title = "Quadbike", Spawnable = true )]
public partial class QuadbikeEntity : Prop, IUse
{
	public static float quadbike_speed_divide = 0.3f; // 0.0f = low -- 0.5f medium -- 1.0f fast	

  	[ConVar.Replicated( "debug_car" )]

	public static bool debug_car { get; set; } = false;

	private QuadbikeWheel frontLeft;
	private QuadbikeWheel frontRight;
	private QuadbikeWheel backLeft;
	private QuadbikeWheel backRight;
	...
	public QuadbikeEntity()
	{
		frontLeft = new QuadbikeWheel( this );
		frontRight = new QuadbikeWheel( this );
		backLeft = new QuadbikeWheel( this );
		backRight = new QuadbikeWheel( this );
	}
	...
	{
		base.Spawn();
		var mdl = "models/cars/Quadbike/quadbike";
		SetModel( mdl );
	...
	public override void ClientSpawn()
	{
...

		{
			...
			chassis_axle_front.LocalPosition = new Vector3( 1.0f, 0, 0.4f ) * 34.0f;
			clientModels.Add( chassis_axle_front );

			{
				wheel0 = new ModelEntity();
				wheel0.SetModel( "models/cars/Quadbike/quadbike_wheel" );
				wheel0.SetParent( chassis_axle_front, "Wheel_Steer_R", new Transform( Vector3.OneX * (-0.1f * 40), Rotation.From( 0, 180, 0 ) ) );
				clientModels.Add( wheel0 );
			}

			{
				wheel1 = new ModelEntity();
				wheel1.SetModel( "models/cars/Quadbike/quadbike_wheel" );
				wheel1.SetParent( chassis_axle_front, "Wheel_Steer_L", new Transform( Vector3.OneX * (0.1f * 40), Rotation.From( 0, 0, 0 ) ) );
...

		{
			...
			chassis_axle_rear.LocalPosition = new Vector3( -1.0f, 0, 0.4f ) * 34.0f;
...

			{
				...
				wheel2.SetModel( "models/cars/Quadbike/quadbike_wheel" );
				wheel2.SetParent( chassis_axle_rear, "Axle_Rear_Center", new Transform( Vector3.Left * (0.8f * 40), Rotation.From( 0, 90, 0 ) ) );
				clientModels.Add( wheel2 );
			}

			{
				wheel3 = new ModelEntity();
				wheel3.SetModel( "models/cars/Quadbike/quadbike_wheel" );
				wheel3.SetParent( chassis_axle_rear, "Axle_Rear_Center", new Transform( Vector3.Right * (0.8f * 40), Rotation.From( 0, -90, 0 ) ) );
	...
	public void OnPrePhysicsStep()
	{
		...
		{
			var forwardSpeed = MathF.Abs( localVelocity.x );
			var speedFactor = quadbike_speed_divide - (forwardSpeed / 5000.0f).Clamp( 0.0f, 1.0f );
